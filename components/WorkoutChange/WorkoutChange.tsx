...
import { styles } from "./WorkoutChange.styles";
...
const WorkoutChange = ({navigation}) => {
...
const allowSave = !nameVal.trim() || !workoutDesc.trim() ||
  workoutList.length === 0 || workoutList.some(ex => ex.sets.length === 0);
...
const user = getAuth().currentUser;
    ...
    createdAt: workout?.createdAt || Date.now(), // preserve old timestamp if editing
  ...
  try {
    ...
    if (workout.docId) {
      ...
      await workoutRef.doc(workout.docId).update(savedData);
 ...
 const handleDeleteSingleSet = (exerciseIdx: number, setIdx: number) => {
  ...
  if (user) {
    ...
    if (userSnap.exists()) {
      ...
      if (data.weight) {
        setWeight(data.weight * 0.453592); // e.g., 72 (kg)
      }
    } else {
...
useEffect(() => {
  if (workout) {
    setNameVal(workout.workoutName || "");
    setWorkoutDesc(workout.workoutDesc || "");
    setWorkoutList(workout.exercises || []);
    setDuration(workout.workoutDuration || "");
    setCalories(workout.caloriesBurned || 0);
  }
}, [workout]);
    ...
    return (
      ...
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.pageHeaderNavs}>
          ...
          <TouchableOpacity style={[styles.trackSaveButtonContainer, allowSave && {opacity: 0.5} ]} onPress={() => handleSave()}>
...
{/* section for the workout details like name and etc */}
        <View style={styles.workoutNameSection}>
          ...
          <View style={{ margin: 5 }}>
            ...
            <TextInput
              style={{ color: "#888", fontWeight: "bold", marginTop: 10}}
              value={workoutDesc}
        ...
        <View style={styles.secondHeader}>
          ...
          <View style={styles.secondHeaderBolds}>
            ...
            <TextInput placeholder="00:00" style={styles.secondHeaderCont} maxLength={5} placeholderTextColor={"white"} value={duration} inputMode="numeric" onChange={handleDuration}/>
              <FontAwesome name="pencil" size={15} color="orange" />
              </View>
            <Text style={styles.statText}>duration</Text>
          </View>
          <View style={styles.secondHeaderBolds}>
            <Text style={[styles.secondHeaderCont, { color: "orange" }]}>{calories > 0 ? calories : "-"}</Text>
      ...
      contentContainerStyle={{
        ...
        >
              ...
              >
                ...
                <View>
                 ...
                 {openMenuIdx === exercise && (
                    <TouchableOpacity activeOpacity={1} style={styles.dropDownHamburger} onPress={(e) => {
          ...
          exercise.previousSets?.[idx]
            ? `${exercise.previousSets[idx].weight || "-"} x ${exercise.previousSets[idx].reps || "-"}`
            : "-"
        }</Text>
                  </View>
                ...
                <View style={styles.weightContain}>
                  <View style={styles.weight}>
                    <TextInput
                      ...
                      onChangeText={(v) =>
                        updateSetField(exIdx, idx, "weight", v)
                      }
                      onFocus={() =>
